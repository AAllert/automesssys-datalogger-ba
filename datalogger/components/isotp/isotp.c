...
int isotp_collect_payload(IsoTpLink *link, uint8_t *payload_buffer, const uint16_t payload_buffer_size, uint16_t *out_size)
{
    uint16_t copylen;
    ...
    if (copylen > payload_buffer_size)
    {
        copylen = payload_buffer_size;
    }

    memcpy(payload_buffer, link->receive_buffer, copylen);
    *out_size = copylen;
...
{
    return ISOTP_RECEIVE_STATUS_FULL == link->receive_status;
}
