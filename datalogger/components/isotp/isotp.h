...
typedef struct IsoTpLink {
    ...
    /* multi-frame flags */
    uint8_t                     send_sn; /* Frame Sequence Number */
    ...
    /* multi-frame control */
    uint8_t                     receive_sn; /* Frame Sequence Number */
 ...
 * @param payload_buffer A pointer to an area in memory where the received data is copied to.
 * @param payload_buffer_size The size of the buffer area.
 ...
 */
int isotp_collect_payload(IsoTpLink *link, uint8_t *payload, const uint16_t payload_size, uint16_t *out_size);

/**
 * @brief Checks if all data has been received.
 * @param link The @link IsoTpLink @endlink instance used to transceive data.
 * @return True (>=1) if all data has been received, false (0) otherwise.
 */
int isotp_received_all_data(IsoTpLink *link);
